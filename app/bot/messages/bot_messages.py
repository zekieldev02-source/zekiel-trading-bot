...
START_MESSAGE = (
    ...
    "\n"
    "⚠️ *Your personal wallet is never used or required.*\n"
    "The bot operates with its own dedicated trading wallet.\n"
    "\n"
    "*To get started (Paper mode):*\n"
    "1️⃣ Set a smart money wallet to copy → /setwallet\n"
    "2️⃣ Set your trade amount → /setamount\n"
    "3️⃣ Start the bot → /startbot\n"
    "\n"
    "*To trade for real (Auto mode):*\n"
    "• Generate the bot's trading wallet → /generatewallet\n"
    "• Send SOL to that wallet\n"
...
HELP_MESSAGE = (
    ...
    "\n"
    "*Copy trading setup:*\n"
    "/setwallet — Set a smart money wallet to copy _(not your own wallet)_\n"
    ...
    "/mode — Switch mode (paper / auto / manual)\n"
    "\n"
    "*Bot trading wallet:*\n"
    "/generatewallet — Create the bot's own trading wallet _(not your personal wallet)_\n"
    "/depositinfo — Show the bot's trading wallet address\n"
    "\n"
    "*Reset:*\n"
...
def get_settings_message(config: UserConfig) -> str:
    ...
    trading_wallet = f"`{config.trading_wallet_public_key}`" if config.trading_wallet_public_key else "➖ Not generated"
    amount = f"{config.trade_amount} SOL" if config.trade_amount else "❌ Not configured"
    tp = f"x{config.tp_multiplier}" if config.tp_multiplier else "➖ Not configured"
    sl = f"{config.stop_loss_multiplier * 100:.0f}%" if config.stop_loss_multiplier else "➖ Not configured"
    ...
    return (
        ...
        "\n"
        f"*Wallet to copy:* {wallet}\n"
        f"*Bot trading wallet:* {trading_wallet}\n"
        f"*Amount per trade:* {amount}\n"
        f"*Take-profit:* {tp}\n"
        f"*Stop-loss:* {sl}\n"
        f"*Max entry MC:* {entry_mc}\n"
